...
const documentColumns: ColumnType<SharedDocument>[] = [
  {
    title: 'Tài liệu',
    key: 'document',
    ...
    onFilter: (value, record) => {
      ...
      const titleMatch = record.title.toLowerCase().includes(search);
      const descMatch = record.description ? record.description.toLowerCase().includes(search) : false;
      const fileMatch = record.fileName ? record.fileName.toLowerCase().includes(search) : false;
      return titleMatch || descMatch || fileMatch; // ✅ luôn boolean
    },
  },
  {
